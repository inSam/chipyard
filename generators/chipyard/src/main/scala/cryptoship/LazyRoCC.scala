...
class AESAccelImp(outer: AESAccel)(implicit p: Parameters) extends LazyRoCCModuleImp(outer) {
  ...
  when (state === s_idle && cmd.valid) {
    ...
    req_counter := 2.U(3.W) //2.U(3.W)
    resp_counter := 2.U(3.W) //2.U(3.W)
    index:= 1.U
    // output(0) := (Cat(aesbb.io.data_out(127 ,120 ), aesbb.io.data_out(119 ,112 ), aesbb.io.data_out(111 ,104 ), aesbb.io.data_out(103 ,96),
    //                   aesbb.io.data_out(95 ,88 ), aesbb.io.data_out(87 ,80 ), aesbb.io.data_out(79 ,72 ), aesbb.io.data_out(71 ,64 )))
    // output(1) := (Cat(aesbb.io.data_out(63 ,56 ), aesbb.io.data_out(55 ,48 ), aesbb.io.data_out(47 ,40 ), aesbb.io.data_out(39 ,32 ),
    //                   aesbb.io.data_out(31 ,24 ), aesbb.io.data_out(23 ,16 ), aesbb.io.data_out(15 ,8 ), aesbb.io.data_out(7 ,0 )))
  ...
  // when (io.mem.req.fire() && state === s_store) {
  //   req_counter := req_counter - 1.U(3.W)
  ...
  //when (io.mem.resp.valid && state === s_load && counter > 0.U) {
